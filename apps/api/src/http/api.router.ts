...
export function createApiRouter(container: Container) {
  ...
  router.use('/auth', authRateLimiter, authHttpRouter(authController))
  router.use('/users', globalRateLimiter, authMiddleware, userHttpRouter(userController))
  ...
  router.use('/bot', globalRateLimiter, authMiddleware, botHttpRouter(botController))
